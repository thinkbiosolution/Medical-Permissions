...
/// @title PatientPermissionRecords
/// Allows Permission Record System to maintain records of patient permissiom in their network.
/// Records can be accessed by CRO if and only if patient provides name.
/// Patients are rewarded with erc20 tokens for providing their name

import "./InterfacePatientPermissionRecords.sol";
import "./SpringToken.sol";
import "./TokenDestructible.sol";
import "./SafeMath.sol";

contract PatientPermissionRecords is InterfacePatientRecords, TokenDestructible {
    ...
    */
    ...
    /// @param __permissionDateStart start date of permiission, simple uint.
    /// @param _permissionDateEnd end date of permission, simple uint.
    /// @param _typeOfMedicalData internal code for type of Medical Data for which permission is given.
    function addRecord (
    ...
    /// @dev Allows a CRO to retrieve the record for a patient.
    /// @param _recordID ID of the patient specific record.
    /// @param _patientAddress address of the patient for record.
    ...
    /// @dev Allows a CRO to view the number of records for a patient.
    /// @param _name Name for the patient
    ...
    /// @dev Allows a CRO to view the number of patients on a given date range.
        ...
        returns (uint _numberOfPatients)
    {
        ...
        for(i = 0; i < recordCount; i++) {
            if(dateRanges[i].permissionDateStart >= from && dateRanges[i].permissionDateEnd <= to)
            _numberOfPatients += 1;
        }
    }
